<h2 align="center">phoenyx </h2>

<h2 align="center">
    <img src="./res/ulfurloyd.jpg" width="240" alt="ulfurloyd" /> <br />
</h2>

![screenshot](./res/screenshot.png)

`phoenyx` is my "ecosystem" for my Linux desktop.
It includes configurations of the applications I use on a daily basis (in this repository)

### Other components:
- ![phoenyx-wallpapers](github.com/ulfurloyd/phoenyx-wallpapers) Wallpapers
- ![phoenyx-media](github.com/ulfurloyd/phoenyx-media) Media server stack (*arr apps)
- ![phoenyx-dwm](github.com/ulfurloyd/phoenyx-dwm) Build of the `dwm` window manager
- ![phoenyx-dmenu](github.com/ulfurloyd/phoenyx-dmenu) Build of the `dmenu` run launcher
- ![phoenyx-dwl](github.com/ulfurloyd/phoenyx-dwl) Build of the `dwl` Wayland compositor

